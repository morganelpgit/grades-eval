...
```{r}
new_df <- full_data |>
  select(id, group, course, grade, sex) |>
  summarise(
    mean_grade = mean(grade, na.rm = TRUE),
    .by = c(id, group, course,sex)
  ) |>
  pivot_wider(
    names_from = course,
...
corr <- new_df |>
  summarise(
    correlation = cor(
      ...
      `Quantum Physics and Time Manipulation`
    ),
    .by = group
  ) 
...
new_df |>
  ...
  ggplot(aes(
    x = `Quantum Physics and Time Manipulation`,
    y = `Energy Harvesting and Sustainability`
  )) +
  geom_point() +
  labs(
    x = "avg grade in QPTM",
...
final_grades <- new_df |>
  mutate(
    final_grade = apply(
      select(new_df, -id, -group, -sex),
      1,
      \(x) mean(x, na.rm = TRUE)
    )
  ) |> 
  select(id,group,final_grade,sex) |>
...
groups are not at the same level.
...
final_grades |>
  ...
  theme_minimal()
```
No clear difference between sex. 
...
last <- final_grades |>
  mutate(
    pass = ifelse(
      if_all(-c(id, group, final_grade), \(x) x >= 5),
      TRUE,
      FALSE
    )
  )
